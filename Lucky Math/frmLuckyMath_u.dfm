object Form4: TForm4
  Left = 0
  Top = 0
  Caption = 'Lucky Math'
  ClientHeight = 183
  ClientWidth = 502
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poDesktopCenter
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object pnlOperator: TPanel
    Left = 95
    Top = 24
    Width = 41
    Height = 41
    TabOrder = 0
  end
  object pnlNum1: TPanel
    Left = 24
    Top = 24
    Width = 41
    Height = 41
    TabOrder = 1
  end
  object pnlNum2: TPanel
    Left = 160
    Top = 24
    Width = 41
    Height = 41
    TabOrder = 2
  end
  object pnlAnswer: TPanel
    Left = 296
    Top = 24
    Width = 41
    Height = 41
    TabOrder = 3
  end
  object pnlEqual: TPanel
    Left = 231
    Top = 24
    Width = 41
    Height = 41
    Caption = '='
    TabOrder = 4
  end
  object pnlOperators: TPanel
    Left = 24
    Top = 71
    Width = 313
    Height = 41
    TabOrder = 5
    object btnMul: TButton
      Left = 162
      Top = 8
      Width = 49
      Height = 25
      Caption = 'x'
      Enabled = False
      TabOrder = 0
      OnClick = btnMulClick
    end
    object btnAdd: TButton
      Left = 16
      Top = 8
      Width = 49
      Height = 25
      Caption = '+'
      Enabled = False
      TabOrder = 1
      OnClick = btnAddClick
    end
    object btnSub: TButton
      Left = 87
      Top = 8
      Width = 49
      Height = 25
      Caption = '-'
      Enabled = False
      TabOrder = 2
      OnClick = btnSubClick
    end
    object btnDiv: TButton
      Left = 243
      Top = 8
      Width = 49
      Height = 25
      Caption = '/'
      Enabled = False
      TabOrder = 3
      OnClick = btnDivClick
    end
  end
  object btnPlay: TButton
    Left = 24
    Top = 118
    Width = 75
    Height = 25
    Caption = 'PLAY !!!'
    TabOrder = 6
    OnClick = btnPlayClick
  end
  object btnSkip: TButton
    Left = 105
    Top = 118
    Width = 75
    Height = 25
    Caption = 'Skip'
    Enabled = False
    TabOrder = 7
    OnClick = btnSkipClick
  end
  object btnStats: TButton
    Left = 186
    Top = 118
    Width = 75
    Height = 25
    Caption = 'Stats'
    TabOrder = 8
    OnClick = btnStatsClick
  end
  object btnReset: TButton
    Left = 267
    Top = 118
    Width = 70
    Height = 25
    Caption = 'Reset'
    TabOrder = 9
    OnClick = btnResetClick
  end
  object memScore: TMemo
    Left = 363
    Top = 71
    Width = 131
    Height = 72
    ScrollBars = ssVertical
    TabOrder = 10
  end
  object btnProcessAnswer: TButton
    Left = 56
    Top = 149
    Width = 25
    Height = 25
    Caption = '='
    TabOrder = 11
    Visible = False
    OnClick = btnProcessAnswerClick
  end
  object btnEqGenerator: TButton
    Left = 24
    Top = 149
    Width = 26
    Height = 26
    Caption = 'btnEqGenerator'
    TabOrder = 12
    OnClick = btnEqGeneratorClick
  end
  object pnlTime: TPanel
    Left = 363
    Top = 24
    Width = 131
    Height = 41
    Caption = '00:00'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -19
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 13
  end
  object btnScoreBoardClear: TButton
    Left = 363
    Top = 149
    Width = 131
    Height = 25
    Caption = 'Clear Score Board'
    TabOrder = 14
    OnClick = btnScoreBoardClearClick
  end
  object tmrGameTimer: TTimer
    Enabled = False
    OnTimer = tmrGameTimerTimer
    Left = 24
    Top = 8
  end
end
